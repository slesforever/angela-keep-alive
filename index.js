    ...
    try {
        ...
        if (channel) {
            const loginEmbed = new EmbedBuilder()
                ...
                .setColor(0x00b4d8)
                .setDescription("「主管，精神脈衝已重新對齊。核心防禦矩陣與影像直顯協定已全面優化。」")
                .addFields(
...
// 📡 自動高頻輪詢 (已優化：純文字廣播，確保 Discord 影片能無阻礙展開)
        ...
        try {
            ...
            if (match) {
                ...
                if (tweetId !== lastFetchedId) {
                    ...
                    const channel = await client.channels.fetch(NOTIFY_CHANNEL_ID);
                    if (channel) {
                        // 🧠 改為純文字格式發送：這樣 vxtwitter 的影片和圖片才不會被 Discord 卡片吃掉
                        const messagePayload = `📢 ${PING_ROLE_MENTION} **【${TARGET_USER.displayName}】**\n\n${tweetContent}\n\n🔗 影片/傳送門：${vxTweetLink}`;
                        await channel.send({ content: messagePayload });
                    }
                }
                break;
            }
...
client.on('messageCreate', async (message) => {
        ...
        for (const nodeUrl of NITTER_NODES) {
            try {
                ...
                if (match) {
                    ...
                    // 🧠 重大修正：這裡「絕對不帶 embeds 卡片」，只吐純文字內文與 vx 網址，給 Discord 空間去渲染推特影片
                    const testPayload = `✅ **報告主管，擷取測試成功！這是官方目前的最新貼文：**\n(廣播頻道：<#${NOTIFY_CHANNEL_ID}>)\n\n💬 **推文內文：**\n${tweetContent}\n\n🎬 **【影片直顯傳送門】**\n${vxTweetLink}`;

                    await message.reply({ content: testPayload });
                    fetchSuccess = true;
                    break;
                }
            } catch (error) {
    ...
    if (msg === '!邊獄人數' || msg === '!limbusonline') {
        try {
            ...
            if (data && data.response && data.response.result === 1) {
                ...
                return message.reply(`📊 **[Steam 即時數據]** 目前共有 **${playerCount.toLocaleString()}** 位罪人正在《Limbus Company》中進行探索。`);
            } else {
    ...
    if (msg === '!狀態' || msg === '!status') {
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields(
                ...
                { name: "📡 監聽機制", value: "1分鐘極速輪詢 (純文字相容版)", inline: true },
    ...
    if (msg === '!ego') {
        const egoList = [
            ...
            { name: "擬態 (Mimicry)", grade: "ALEPH", desc: "由血肉扭曲而成的巨大刀刃。這代表個體擅長在不同環境中偽裝、完美貼上符合群體需求的標籤，然而真實的自我正在逐漸被吞噬。" }
        ];
    ...
    if (msg === '!逆流') {
        const alarmEmbed = new EmbedBuilder()
            ...
            .addFields(
                ...
                { name: "🛠️ 處置方針", value: "請立刻停止認知扭曲行為，回歸本職工作。", inline: false }
            )
    ...
    if (msg.startsWith('!尋找機器人') || msg.startsWith('!findbot')) {
        ...
        try {
            ...
            if (foundBots.size === 0) return message.reply(`🔍 找不到名字包含「${args.slice(1).join(' ')}」的機器人。`);
