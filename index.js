...
// 監聽伺服器錯誤，防止 EADDRINUSE 直接把整台機器人弄崩潰
server.on('error', (err) => {
    if (err.code === 'EADDRINUSE') {
        console.error(`⚠️ [網路警告] 連接埠 ${PORT} 已被佔用。機器人將跳過網頁綁定，繼續嘗試啟動核心 Discord 服務...`);
        console.error(`👉 如果是在雲端部署，請至控制台點選 "Clear Cache & Deploy" 來釋放連接埠。`);
    } else {
